...
void printTutorials(TemplateArray<Tutorial> v) {
...
{
	TemplateArray<Tutorial> v = this->ctrl.getRepo().getTutorials();
	Tutorial* tuts = v.getAllElems();
	if (tuts == NULL)
		return;
	if (v.getSize() == 0)
	{
...
{
	TemplateArray<Tutorial> v = this->ctrl.getWatchList().getTuts();
	Tutorial* tuts = v.getAllElems();
	if (tuts == NULL)
		return;
	if (v.getSize() == 0)
