...
vector<Tutorial> Repository::filterByPresenter(std::string presenter) {
...
Tutorial Repository::getCurrentTut() {
	if (this->current == filterList.size())
		this->current = 0;
...
void Repository::playTutorials() {
	if (this->filterList.size() == 0)
		return;
	this->current = 0;
	Tutorial currentTut = this->getCurrentTut();
	currentTut.play();
