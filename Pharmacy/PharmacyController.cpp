...
PharmacyController::PharmacyController() {
    //this->repo = repo;
    //validator = val;
    //unique_ptr <PharmacyRepo> repo (new PharmacyRepo());
    this->repo= std::move(repo);
}

PharmacyController::PharmacyController(unique_ptr<PharmacyRepo> repo, unique_ptr<MedicineValidator> val) {
    this->repo = std::move(repo);
 ...
 */
void PharmacyController::undoController(){
    repo->undoL();
}
...
/*
 * filter by name, uses lambda function
 * prec: string name
...
vector<Medicine> PharmacyController::filterByQuantity(int quant) {
	...
	return rez;
	*/
    vector<Medicine> rez = vector<Medicine>();
    vector<Medicine> all = repo->getAll();
    rez.resize(all.size());
    auto it = remove_copy_if(all.begin(), all.end(), rez.begin(), FQuant(quant));
    rez.erase(it, rez.end());
	return rez;
}
